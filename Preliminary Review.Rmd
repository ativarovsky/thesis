...
```{r survey design variables}
# create variable for 4-year weighting
model_data$mec4yr = model_data$wtmec2yr/2

# create survey design adjustment variable
...
# subset for age and gender
ageDesign = subset(nhanesDesign, riagendr == 2 & ridageyr > 17 & ridageyr < 45 & !indfmin2 %in% c(77,99))

```


```{r models for phenols}

svymean(~ridageyr, ageDesign)

output = svyglm(log(urxbp3) ~ ridageyr + factor(ridreth3) + factor(dmdeduc2) + factor(dmdmartl) + factor(indfmin2),
          family = gaussian(),
          data   = model_data,
          design = ageDesign)
...
model_1 = lm(log(urxbp3) ~ ridageyr + factor(ridreth3) + factor(dmdeduc2) + factor(dmdmartl) + factor(indfmin2), data = model_raw) 
