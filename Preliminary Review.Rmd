...
First we apply the survey weight to the separate datasets. We create 

```{r survey weights}

# tidy all_data for survey analysis
all_data = 
  all_data %>% 
  janitor::clean_names()
         
# create variable for 4-year weighting
all_data$mec4yr = all_data$wtmec2yr/2
...
ageDesign = subset(nhanesDesign, riagendr == 2 & ridageyr > 17 & ridageyr < 45)

```


```{r tidying for analysis}

svymean(~ridageyr, ageDesign, na.rm = TRUE)

output = svyglm(urxbp3 ~ ridageyr,
          family = gaussian(),
          data   = all_data,
          design = ageDesign)
